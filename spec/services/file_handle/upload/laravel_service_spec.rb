RSpec.describe FileHandle::Upload::LaravelService do
  describe '.initialize' do
    context 'if file does not exist' do
      let!(:task_without_file) { create :task }

      it 'raises error' do
        expect { FileHandle::Upload::LaravelService.new(task: task_without_file) }.to raise_error(StandardError)
      end
    end

    context 'if file exists' do
      let!(:task) { create :task, :with_json_keyed }
      let(:loader) { FileHandle::Upload::LaravelService.new(task: task) }
  ...
  describe '.locale_is_correct?' do
    context 'for incorrect locale' do
      let!(:task) { create :task, :with_wrong_json_keyed }
      let(:loader) { FileHandle::Upload::LaravelService.new(task: task) }
  ...
  describe '.task_update' do
    ...
    context 'for locale like ru' do
      it 'updates task with ru locale' do
        expect { loader.send(:task_update) }.to change(task, :from).to('de')
      end
    end
  end
  ...
  describe '.locale' do
    context 'for files with standard locale' do
      ...
      it 'returns default de locale' do
        expect(loader.send(:locale)).to eq 'de'
      end
    end
  end
