RSpec.describe FileHandle::Upload::NetService do
  describe '.initialize' do
    context 'if file does not exist' do
      let!(:task_without_file) { create :task }

      it 'raises error' do
        expect { FileHandle::Upload::NetService.new(task: task_without_file) }.to raise_error(StandardError)
      end
    end

    context 'if file exists' do
      let!(:task) { create :task, :with_resx }
  ...
  describe '.locale' do
    ...
    context 'for files with locale' do
      let!(:task) { create :task, :with_long_resx }
      let(:loader) { FileHandle::Upload::NetService.new(task: task) }
