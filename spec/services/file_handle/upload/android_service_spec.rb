RSpec.describe FileHandle::Upload::AndroidService do
  describe '.initialize' do
    context 'if file does not exist' do
      let!(:task_without_file) { create :task }

      it 'raises error' do
        expect { FileHandle::Upload::AndroidService.new(task: task_without_file) }.to raise_error(StandardError)
      end
    end

    context 'if file exists' do
      let!(:task) { create :task, :with_xml }
      ...
      it 'assigns data from file to @uploaded_file' do
        expect(loader.uploaded_file.is_a?(Nokogiri::XML::Document)).to eq true
      end
    end
  end
  ...
  describe '.load' do
    context 'for correct situations' do
      ...
      it 'returns hash with values' do
        expect(loader.load.is_a?(Nokogiri::XML::Document)).to eq true
      end
    end
  end
  ...
  describe '.locale_is_correct?' do
    context 'for incorrect locale' do
      let!(:task) { create :task, :with_wrong_xml }
      let(:loader) { FileHandle::Upload::AndroidService.new(task: task) }
  ...
  describe '.locale' do
    ...
    context 'for files with locale' do
      let!(:task) { create :task, :with_locale_xml }
      let(:loader) { FileHandle::Upload::AndroidService.new(task: task) }
