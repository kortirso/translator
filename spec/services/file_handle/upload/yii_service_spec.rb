RSpec.describe FileHandle::Upload::YiiService do
  describe '.initialize' do
    context 'if file does not exist' do
      let!(:task_without_file) { create :task }

      it 'raises error' do
        expect { FileHandle::Upload::YiiService.new(task: task_without_file) }.to raise_error(StandardError)
      end
    end

    context 'if file exists' do
      let!(:task) { create :task, :with_php }
  ...
  describe '.locale' do
    ...
    context 'for files with locale' do
      let!(:task) { create :task, :with_locale_php }
      let(:loader) { FileHandle::Upload::YiiService.new(task: task) }
